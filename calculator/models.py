...
class Matrix(models.Model):
    ...
    name = models.CharField(max_length=200, help_text="Nombre descriptivo de la matriz")
    rows = models.PositiveIntegerField(
        validators=[MinValueValidator(1)], help_text="Número de filas"
    )
    cols = models.PositiveIntegerField(
        validators=[MinValueValidator(1)], help_text="Número de columnas"
    )
...
class Operation(models.Model):
    ...
    operation_type = models.CharField(
        max_length=20, choices=OPERATION_TYPES, help_text="Tipo de operación realizada"
    )
    ...
    def __str__(self):
        if self.matrix_b:
            return (
                f"{self.get_operation_type_display()}: {self.matrix_a.name} y {self.matrix_b.name}"
            )
