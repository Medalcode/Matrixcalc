...
from calculator.utils import InvalidMatrixError, NumericError

# --- Helpers de Operación ---


from calculator.services import OperationService


def _perform_matrix_operation(operation_type, matrix_a_id, matrix_b_id=None, extra_data=None):
    """
    Helper de vista HTTP que delega la ejecución de operaciones matriciales al OperationService.
    """
    ...
    try:
        ...
        return Response(
            {"error": "Una o ambos matrices no existen"}, status=status.HTTP_404_NOT_FOUND
        )

    try:
        operation = OperationService.execute_matrix_operation(
...
def ratelimit_viewset(rate):
    def decorator(cls):
        ...
        cls = method_decorator(
            ratelimit(key="ip", rate=rate, method="PATCH"), name="partial_update"
        )(cls)
...
class MatrixViewSet(viewsets.ModelViewSet):
    ...
    def export_csv(self, request, pk=None):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="matrix_{matrix.id}_{matrix.name}.csv"'
        )
        return response
    ...
    def import_csv(self, request):
        ...
        if "file" not in request.FILES:
            return Response(
                {"error": "No se proporcionó archivo"}, status=status.HTTP_400_BAD_REQUEST
            )

        file = request.FILES["file"]
        name = request.data.get(
            "name", f"Matriz importada {timezone.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        try:
            ...
            if not lines:
                return Response(
                    {"error": "El archivo CSV está vacío"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            # Crear matriz
            matrix = Matrix.objects.create(name=name, rows=rows, cols=cols, data=data)
...
def sum_matrices(request):
    ...
    return _perform_matrix_operation(
        "SUM", request.data.get("matrix_a_id"), request.data.get("matrix_b_id")
    )
...
def subtract_matrices(request):
    """Resta dos matrices."""
    return _perform_matrix_operation(
        "SUBTRACT", request.data.get("matrix_a_id"), request.data.get("matrix_b_id")
    )
...
def multiply_matrices(request):
    ...
    return _perform_matrix_operation(
        "MULTIPLY", request.data.get("matrix_a_id"), request.data.get("matrix_b_id")
    )
...
def stats_view(request):
    ...
    operations_by_type = (
        Operation.objects.values("operation_type")
    ...
    avg_exec_time = Operation.objects.aggregate(avg_time=Avg("execution_time_ms"))["avg_time"] or 0

    # Operaciones recientes (últimos 7 días)
    seven_days_ago = timezone.now() - timedelta(days=7)
    recent_operations_count = Operation.objects.filter(created_at__gte=seven_days_ago).count()
