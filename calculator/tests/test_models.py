...
class TestMatrixModel:
    ...
    def test_matrix_validation_rows_cols(self):
        ...
        with pytest.raises(ValidationError):
            matrix = Matrix(name="Invalid", rows=0, cols=3, data=[[1, 2, 3]])
            matrix.full_clean()

    def test_matrix_empty_data(self):
        """Test creating matrix with empty data"""
        matrix = Matrix.objects.create(name="Empty", rows=2, cols=2, data=[])
        assert matrix.data == []
...
class TestOperationModel:
    ...
    def test_create_operation(self, matrix_pair):
        ...
        result = Matrix.objects.create(name="Result", rows=2, cols=2, data=[[6, 8], [10, 12]])
    ...
    def test_operation_str_representation(self, matrix_pair):
        ...
        matrix_a, matrix_b = matrix_pair
        result = Matrix.objects.create(name="Result", rows=2, cols=2, data=[[19, 22], [43, 50]])
        operation = Operation.objects.create(
    ...
    def test_unary_operation(self, matrix):
        ...
        result = Matrix.objects.create(
            name="Transposed", rows=3, cols=3, data=[[1, 4, 7], [2, 5, 8], [3, 6, 9]]
        )
        operation = Operation.objects.create(
    ...
    def test_binary_operation_check(self, matrix_pair):
        ...
        matrix_a, matrix_b = matrix_pair
        result = Matrix.objects.create(name="Sum Result", rows=2, cols=2, data=[[6, 8], [10, 12]])
        operation = Operation.objects.create(
            operation_type="SUM",
            matrix_a=matrix_a,
            matrix_b=matrix_b,
