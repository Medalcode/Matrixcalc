...
def sample_matrix_data():
    ...
    return {"name": "Test Matrix", "rows": 3, "cols": 3, "data": [[1, 2, 3], [4, 5, 6], [7, 8, 9]]}
...
def identity_matrix(db):
    ...
    return Matrix.objects.create(  # type: ignore[attr-defined]
        name="Identity 3x3", rows=3, cols=3, data=[[1, 0, 0], [0, 1, 0], [0, 0, 1]]
    )
...
def matrix_pair(db):
    ...
    matrix_a = Matrix.objects.create(  # type: ignore[attr-defined]
        name="Matrix A", rows=2, cols=2, data=[[1, 2], [3, 4]]
    )
    matrix_b = Matrix.objects.create(  # type: ignore[attr-defined]
        name="Matrix B", rows=2, cols=2, data=[[5, 6], [7, 8]]
    )
