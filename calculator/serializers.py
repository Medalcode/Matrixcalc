...
from calculator.utils import InvalidMatrixError, parse_matrix
        ...
        if data is not None:
            for i, row in enumerate(data):
                if not isinstance(row, list):
                    raise serializers.ValidationError(f"La fila {i} debe ser una lista.")
...
class StatsSerializer(serializers.Serializer):
    ...
    operations_by_type = serializers.ListField(child=serializers.DictField())
    operations_timeline = serializers.ListField(child=serializers.DictField())
